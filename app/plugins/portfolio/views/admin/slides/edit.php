<?php $this->tplDisplay("inc_header.php", ['menu'=>'clients','sPageTitle'=>"Portfolio &raquo; ".$aClient['name']." &raquo; Create Slide"]); ?>
  ...
  <form id="add-form" method="post" action="/admin/portfolio/<?= $aClient['id'] ?>/slides/edit/s/" enctype="multipart/form-data">
    <div class="row-fluid">
      <div class="span8">
              ...
              foreach($images as $key=>$image): ?>
                <?php if(!empty($aSlide[$key])): ?>
                  <div class="control-group photo-show">
                    <img src="<?php echo $aSlide[$key."_url"]; ?>" alt="<?= $image['title'] ?>" style="max-width: <?= $image['max-width'] ?>px;"><br />
